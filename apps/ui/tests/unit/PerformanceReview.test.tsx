...
vi.mock("react-chartjs-2", () => ({
  Chart: (props: { type: string; data: unknown }) => (
    <canvas
      data-testid={`chartjs-${props.type}`}
      data-datasets={
        Array.isArray((props.data as { datasets?: unknown[] }).datasets)
          ? (props.data as { datasets: unknown[] }).datasets.length
          : 0
      }
    />
  ),
...
describe("Performance review", () => {
  ...
  it("T-P3 agent scorecard shows 8 rows with real call counts", async () => {
    ...
    const draft = card.querySelector(
      'tr[data-agent="DraftComposer"] td:nth-child(2)',
    );
