...
describe("Integration · loading, errors, empty states", () => {
    it("T-I1 shows a loading state while fetching", async () => {
        let resolve;
        vi.spyOn(runs, "getRun").mockImplementation(() => new Promise((r) => {
            resolve = r;
        }));
        renderWithRouter({
    ...
    it("T-I2 renders error card with retry that refetches", async () => {
        ...
        await waitFor(() => expect(screen.getByTestId("employee-console")).toBeInTheDocument());
        expect(spy).toHaveBeenCalledTimes(2);
    });
    it("T-I3 unknown :runId shows 'Run not found' empty state", async () => {
        vi.spyOn(runs, "getRun").mockRejectedValue(new ApiError("Run not found", 404, "/api/v1/runs/none"));
        renderWithRouter({
